...
class VectorStoreManager:
    def __init__(self, input_dir, chunk_size, chunk_overlap):
        data = SimpleDirectoryReader(input_dir=input_dir).load_data()
        embed_model = MyEmbeddings()
        tokenizer = tiktoken.encoding_for_model("gpt-4o-mini").encode
        splitter = TokenTextSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap, tokenizer=tokenizer)
...
input_dir = "/prompt_engineering/docs/"
model = "gpt-4o-mini"
...
index = VectorStoreManager(input_dir, chunk_size=chunk_size, chunk_overlap=chunk_overlap).index
